...
cp2_gibbs_v2 <- function(data, iter, start.vals, prop_var, cp_prop_var, tol = 10, warmup = 5000, verbose = FALSE)
{
  ...
  for(i in 1:(warmup))
  {
    ...
    for(j in 1:3)
    {
      if(j == 1)
      {
        prop <- as.numeric(rmvnorm(n = 1, mean = c(sigma[j], l[j], tau[j], beta[1], intercept[1]), sigma = prop_var[[j]]))
      }
      if(j == 3)
      {
        prop <- as.numeric(rmvnorm(n = 1, mean = c(sigma[j], l[j], tau[j], beta[2], intercept[2]), sigma = prop_var[[j]]))
      }
      if(j == 2)
      {
