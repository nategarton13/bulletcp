...
```{r raw_data, echo=TRUE, cache=TRUE, fig.cap="\\label{f:raw_data} A plot of an averaged cross cut from a 3D bullet land scan."}
library(ggplot2)
raw_data <- read.csv(file = "C:/Users/Nate/Documents/CSAFE/Bullets/bulletcp/bulletcp/example_data.csv")
head(raw_data)
...
```{r global_structure, echo=TRUE, cache=TRUE}
...
Once the data have been scaled, we take a subset of them by selecting every 20-th point to use when estimating the Gaussian process parameters. Then, we estimate these parameters using the ```mlgp()``` function in this package. 
```{r estimate_gp_par, echo = TRUE, cache=TRUE}
## load the bulletcp package
library(bulletcp)
