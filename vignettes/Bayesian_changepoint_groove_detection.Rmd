...
Now we introduce the usage of a function which, once global structure has been removed, will scale the data and impute missing values. The function is named ```detect_cp_v2()```. Notice that the ```data``` argument here is set to be the 

```{r detect_cp, echo = TRUE, cache = FALSE}
